...
class SolveAllenCahn:
    ...
    def __init__(self, sess):
        ...
        self.h = (self.T+0.0)/self.n_time  # TODO: why +0.0??
    ...
    def train(self):
        ...
        print('step: {:5d}, loss: {:.4e}, '.format(0, temp_loss) +
              'Y0: {:.4e}, runtime: {:4f} '.format(temp_init, time.time()-start_time+self.t_bd))
        ...
        for _ in range(self.n_maxstep+1):
            ...
            if step % self.n_displaystep == 0:
                 ...
                 self.loss_history.append(temp_loss)
                 self.init_history.append(temp_init)
                 print('step: {:5d}, loss: {:.4e}, '.format(step, temp_loss) +
                       'Y0: {:.4e}, runtime: {:4f} '.format(temp_init, time.time()-
                                                           start_time+self.t_bd))
            step += 1
        end_time = time.time()
        print('running time: {:.3f}s'.format(end_time-start_time+self.t_bd))
    ...
    def build(self):
        ...
        self.Y0 = tf.Variable(tf.random_uniform([1], minval=self.Yini[0],
                                                maxval=self.Yini[1], dtype=tf.float64))
        self.Z0 = tf.Variable(tf.random_uniform([1, self.d], minval=-.1,
                                                maxval=.1, dtype=tf.float64))
    ...
    def sample_path(self, n_sample):
        ...
        for i in range(self.n_time):
            dW_sample[:, :, i] = np.reshape(normal.rvs(mean=np.zeros(self.d), cov=1, size=n_sample) *
                                            self.sqrth, (n_sample, self.d))
    ...
    def g_tf(self, t, X):
        # terminal conditions
        return 0.5/(1 + 0.2*tf.reduce_sum(X**2, 1, keepdims=True))
